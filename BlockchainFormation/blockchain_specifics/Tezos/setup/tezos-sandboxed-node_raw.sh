...
start_sandboxed_node() {
    ...
    expected_connections=substitute_number_of_peers
    node_dir="/home/ubuntu/tezos_node_dir"
    substitute_string_peers
    ...
    if ! [ -f "${node_dir}/config.json" ]; then
        if [ -n "$USE_TLS" ]; then
            $node config init \
                  --network "sandbox" \
                  --data-dir "$node_dir" \
                  --net-addr "substitute_priv_ip:$port" \
                  --rpc-addr "substitute_priv_ip:$rpc" \
