...
def indy_shutdown(config, logger, ssh_clients, scp_clients):
    ...
    for index, _ in enumerate(config['priv_ips']):
        ...
        stdin, stdout, stderr = ssh_clients[index].exec_command("screen -list")
...
def indy_restart(config, ssh_clients, scp_clients, logger):

    indy_shutdown(config, ssh_clients, scp_clients, logger)
    indy_startup(config, ssh_clients, scp_clients, logger)
