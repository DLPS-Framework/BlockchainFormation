...
class BenchContract extends Contract {
    ...
    async measurePutState(ctx, key, value, count) {
        console.info('============= START : measurePutState ===========');
        const start = Date.now()

        const warmupCount = parseInt(count * 0.1) + 1;
        console.info(`Transaction ID: ${ctx.stub.getTxID()}, Execute ${warmupCount} warmup putState ops`);
        for (var i = 0; i < warmupCount; i++) {
            await ctx.stub.putState("key_" + key, Buffer.from("value_" + value));
        }

        const startMeasurementAt = Date.now()
        console.info(`Transaction ID: ${ctx.stub.getTxID()}, Execute ${count} putState ops`);
        for (var i = 0; i < count; i++) {
            await ctx.stub.putState("key_" + key, Buffer.from("value_" + value));
        }
        const duration = (Date.now()-startMeasurementAt) / count;

        console.info("============= END : measurePutState =========== after " + (Date.now()-start).toString() + "ms, delay " + duration + "ms");
        return Buffer.from(duration.toString()); // not sure if I need to return here a string
    }
